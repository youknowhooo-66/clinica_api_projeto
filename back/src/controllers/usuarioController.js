...
const getUsuarios = async (req, res) => {
	try {
		const [resultado] = await db.query("SELECT id, nome, cpf, senha, tipo_usuario FROM usuario ");
...
const editarUsuarios = async (req, res) => {
	try {
		const [resultado] = await db.query("UPDATE usuario SET nome = ?, cpf = ?, WHERE id = ?", [req.body.nome, req.body.cpf, req.params.id]);
...
const adicionarUsuario = async (req, res) => {
    try {
		...
		const [resultado] = await db.query("INSERT INTO usuario (nome, cpf, email, senha, tipo_usuario) VALUES (?, ?, ?, ?)", [nome, cpf, email, senha, tipo_usuario]);
