...
const PatientDetails = () => {
  ...
  const handleUpdateConsult = async event => {
    ...
    try {
      // Verifique se há uma consulta selecionada para edição
      if (!editingConsult) {
        return
      }

      // Crie um objeto com os dados atualizados da consulta
      ...
      // Fazendo a chamada PUT para atualizar os dados da consulta
      await axios.put(
        `http://localhost:3000/consults/${editingConsult.id}`,
        updatedConsult
      )

      // Atualize a lista de consultas para refletir a alteração
      ...
      // Limpe os estados de edição após a atualização
  ...
  const handleDeleteConsult = async (consultId) => {
    try {
      // Fazendo a chamada DELETE para excluir a consulta
  ...
  const handleUpdateExam = async event => {
    ...
    try {
      // Verifique se há um exame selecionado para edição
      if (!editingExam) {
        return;
      }

      // Crie um objeto com os dados atualizados do exame
      const updatedExam = {
        ...editingExam,
      ...
      // Fazendo a chamada PUT para atualizar os dados do exame
      await axios.put(
        `http://localhost:3000/exams/${editingExam.id}`,
        updatedExam
      );

      // Atualize a lista de exames para refletir a alteração
      setExams(prevExams =>
  ...
  const handleDeleteExam = async (examId) => {
    try {
      // Fazendo a chamada DELETE para excluir o exame
      await axios.delete(`http://localhost:3000/exams/${examId}`);

      // Atualize a lista de exames após a exclusão
      setExams((prevExams) =>
